...
/** Realistic fallback AI quote — keyword-driven line items using 2026 UK pricing. */
export const generateFallbackQuote = (description: string, vatRegistered: boolean): {
  title: string;
  line_items: LineItem[];
} => {
  const d = description.toLowerCase();
  const items: LineItem[] = [];
  let title = "Plumbing & heating works";

  const has = (...keys: string[]) => keys.some((k) => d.includes(k));

  if (has("boiler") && has("install", "fit", "replace", "swap", "new")) {
    title = "Combi boiler — supply & fit";
    items.push(
      { description: "Worcester Bosch Greenstar 30i combi boiler", qty: 1, unit_price: 1395 },
      { description: "Magnaclean Pro2 magnetic filter", qty: 1, unit_price: 145 },
      { description: "Full system power flush", qty: 1, unit_price: 395 },
  ...
  } else if (has("boiler", "service")) {
    title = "Annual boiler service";
    items.push(
      { description: "Annual boiler service — combi", qty: 1, unit_price: 95 },
      { description: "Gas Safe certificate", qty: 1, unit_price: 55 },
    );
  } else if (has("bathroom", "shower", "suite", "bath ")) {
    title = "Bathroom refit — supply & fit";
    items.push(
      { description: "Bathroom suite (bath, basin, WC, shower)", qty: 1, unit_price: 1850 },
      { description: "Thermostatic shower valve & enclosure", qty: 1, unit_price: 485 },
      { description: "Wall & floor tiles + adhesive/grout", qty: 1, unit_price: 620 },
      { description: "Strip out, dispose & first-fix plumbing", qty: 1, unit_price: 420 },
      { description: "Labour — 2 days (lead + mate)", qty: 2, unit_price: 540 },
      { description: "Sundries, sealant, isolators", qty: 1, unit_price: 105 },
    );
  } else if (has("radiator", "rad ", "trv")) {
    title = "Radiator replacement & TRVs";
    items.push(
      { description: "Double-panel convector radiator 1200x600", qty: 2, unit_price: 145 },
      { description: "Drayton TRV4 thermostatic valves", qty: 4, unit_price: 24 },
      { description: "System drain, flush & inhibitor", qty: 1, unit_price: 95 },
      { description: "Labour — 1 day", qty: 1, unit_price: 215 },
    );
  } else if (has("underfloor", "ufh")) {
    title = "Underfloor heating installation";
    items.push(
      { description: "UFH manifold + pump & blending valve", qty: 1, unit_price: 385 },
      { description: "UFH PE-RT pipe — 120m", qty: 1, unit_price: 165 },
      { description: "Wiring centre + room thermostat", qty: 1, unit_price: 145 },
      { description: "Labour — 2 days fit & commission", qty: 2, unit_price: 540 },
    );
  } else if (has("thermostat", "smart stat", "nest", "hive", "honeywell")) {
    title = "Smart thermostat — supply & fit";
    items.push(
      { description: "Honeywell Home T6R wireless thermostat kit", qty: 1, unit_price: 195 },
      { description: "Installation, wiring & commissioning", qty: 1, unit_price: 125 },
    );
  } else if (has("leak", "burst", "pipe")) {
    title = "Leak / burst pipe repair";
    items.push(
      { description: "Emergency call out", qty: 1, unit_price: 95 },
      { description: "Push-fit fittings & 1m copper", qty: 1, unit_price: 38 },
      { description: "Labour on site (2hr)", qty: 2, unit_price: 68 },
    );
  } else if (has("tap", "mixer", "faucet")) {
    title = "Tap / mixer replacement";
    items.push(
      { description: "Bristan basin mixer (chrome)", qty: 1, unit_price: 85 },
      { description: "Isolation valves & sundries", qty: 1, unit_price: 18 },
      { description: "Labour (1hr)", qty: 1, unit_price: 65 },
    );
  } else if (has("toilet", "wc", "cistern")) {
    title = "Toilet / cistern works";
    items.push(
      { description: "Dual-flush close-coupled cistern + pan", qty: 1, unit_price: 215 },
      { description: "Wax seal, isolators, sundries", qty: 1, unit_price: 28 },
      { description: "Labour (2hr)", qty: 2, unit_price: 65 },
    );
  } else if (has("gas safe", "cp12", "landlord", "safety cert")) {
    title = "Landlord gas safety certificate";
    items.push(
      { description: "Landlord CP12 inspection", qty: 1, unit_price: 75 },
      { description: "Report admin", qty: 1, unit_price: 25 },
    );
  } else if (has("power flush", "flush")) {
    title = "Full system power flush";
    items.push(
      { description: "Power flush — up to 8 radiators", qty: 1, unit_price: 395 },
      { description: "Inhibitor + magnetic filter check", qty: 1, unit_price: 65 },
    );
  } else {
    // Generic plumbing call out
    title = "Plumbing call out & works";
    items.push(
      { description: "Diagnostic call out", qty: 1, unit_price: 65 },
      { description: "Parts & sundries (estimated)", qty: 1, unit_price: 85 },
      { description: "Labour (2hr)", qty: 2, unit_price: 65 },
    );
  }
  return { title, line_items: items };
};
