...
import { mockQuotes, getClient, mockProfile, formatGBP } from "@/lib/mock-data";
    ...
    <AppShell>
      ...
      <section className="px-5 mt-5 space-y-3">
        {overdue.map((q) => {
          ...
          const mail = `mailto:${c?.email}?subject=${encodeURIComponent(`Overdue: ${q.ref}`)}&body=${chase}`;
          return (
            <div key={q.id} className="card-surface p-4">
              ...
              <div className="grid grid-cols-3 gap-2 mt-4">
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="bg-lime text-ink rounded-full py-2.5 text-xs font-bold inline-flex items-center justify-center gap-1.5"
                >
                ...
                <a
                  href={`tel:${c?.phone}`}
                  className="bg-ink text-paper rounded-full py-2.5 text-xs font-bold inline-flex items-center justify-center gap-1.5"
                >
                ...
                <a
                  href={mail}
                  className="bg-card border border-border text-ink rounded-full py-2.5 text-xs font-bold inline-flex items-center justify-center gap-1.5"
                >
